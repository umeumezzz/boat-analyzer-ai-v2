...
async function getOriginal(jcd,hd,rno){
...
export async function GET(req){
 ...
 try{
  ...
  const [rr,bb,oo]=await Promise.allSettled([grab(`racelist?hd=${hd}&jcd=${jcd}&rno=${rno}`,45),grab(`beforeinfo?hd=${hd}&jcd=${jcd}&rno=${rno}`,20),getOriginal(jcd,hd,rno)]);
  ...
  const race=parseRace(rr.value),commonBefore=bb.status==='fulfilled'?parseBefore(bb.value):{available:false,rows:[],weather:{}},original=oo.status==='fulfilled'?oo.value:{supported:false,available:false,rows:[]},before=mergeBefore(commonBefore,original);
